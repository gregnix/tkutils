...
proc ::sqledit::_menu {toplevel} {
    ...
    # Zuletzt geoeffnet -- nur wenn sqledit-recent.tcl geladen wurde.
    #
    # Das Untermenue wird bei JEDEM Aufklappen neu gefuellt
    # (-postcommand), sonst fehlte das gerade geoeffnete Ziel bis zum
    # naechsten Start.
    if {[llength [info procs ::sqledit::recent::menueFuellen]]} {
        set rec [menu $file.recent -tearoff 0 -postcommand \
                [list ::sqledit::recent::menueFuellen $file.recent \
                      ::sqledit::_connectTo]]
        $file add cascade -label "Zuletzt geoeffnet" -menu $rec
    }
    $file add separator
...
proc ::sqledit::_connectTo {target} {
    ...
    _retitle
    # ERST HIER merken, nicht schon beim Auswaehlen: was sich nicht
    # oeffnen liess, gehoert nicht in die Liste der zuletzt geoeffneten.
    if {[llength [info procs ::sqledit::recent::merken]]} {
        ::sqledit::recent::merken $target
    }
}
