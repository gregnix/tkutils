# tkutils::tkdateentry -- date entry with a drop-down calendar picker.
#
...
proc ::tkutils::tkdateentry::widget {path args} {
    variable state
    ...
    bind $path <Destroy> [list ::tkutils::tkdateentry::_cleanup $path %W]
    ...
    ttk::button $path.b -text "\u25BC" -width 2 \
        -command [list ::tkutils::tkdateentry::_toggle $path]
...
proc ::tkutils::tkdateentry::getText {path} { return [$path.e get] }

# ISO date (yyyy-mm-dd) parsed from the entry text via -dateformat, or "".
proc ::tkutils::tkdateentry::getDate {path} {
    variable state
...
proc ::tkutils::tkdateentry::setDate {path iso} {
    variable state
...
proc ::tkutils::tkdateentry::today {path} {
    return [setDate $path [clock format [clock seconds] -format %Y-%m-%d]]
}

proc ::tkutils::tkdateentry::clear {path} { $path.e delete 0 end; return "" }

# --- popup ---------------------------------------------------------------

proc ::tkutils::tkdateentry::_toggle {path} {
    if {[winfo exists $path.pop]} { _hide $path } else { _show $path }
}

proc ::tkutils::tkdateentry::_hide {path} {
    catch {grab release $path.pop}
    catch {destroy $path.pop}
}

proc ::tkutils::tkdateentry::_show {path} {
    variable state
    ...
    _render $path
    bind $pop <Escape> [list ::tkutils::tkdateentry::_hide $path]
    # close when focus leaves the popup
    bind $pop <FocusOut> [list ::tkutils::tkdateentry::_focusOut $path]
    catch {grab -global $pop}
    focus $pop
}

proc ::tkutils::tkdateentry::_focusOut {path} {
    set pop $path.pop
...
proc ::tkutils::tkdateentry::_nav {path delta} {
    variable state
...
proc ::tkutils::tkdateentry::_render {path} {
    variable state
    variable WD
    set f $path.pop.f
    ...
    ttk::button $f.h.p -text "\u25C0" -width 2 \
        -command [list ::tkutils::tkdateentry::_nav $path -1]
    ...
    for {set d 1} {$d <= $dim} {incr d} {
        ...
        ttk::button $b -text $d -width 3 \
            -command [list ::tkutils::tkdateentry::_pick $path $iso]
...
proc ::tkutils::tkdateentry::_pickClear {path} {
    variable state
    clear $path
    _hide $path
    if {$state($path,cmd) ne ""} {
...
package provide tkutils::tkdateentry 0.1
