# tkutils::tkdavaccount -- a DAV account form (URL / user / password / type)
...
namespace eval ::tkutils::tkdavaccount {
    namespace export widget getConfig setConfig clear testConnection focusUrl
    variable state
}

proc ::tkutils::tkdavaccount::_cleanup {path w} {
    variable state
...
proc ::tkutils::tkdavaccount::widget {path args} {
    variable state
    ...
    set ns ::tkutils::tkdavaccount::state
    ...
    bind $path <Destroy> [list ::tkutils::tkdavaccount::_cleanup $path %W]
    ...
    ttk::button $path.test -text "Test connection" \
        -command [list ::tkutils::tkdavaccount::testConnection $path]
...
proc ::tkutils::tkdavaccount::getConfig {path} {
    variable state
    return [dict create \
        url      $state($path,url) \
...
proc ::tkutils::tkdavaccount::setConfig {path cfg} {
    variable state
...
proc ::tkutils::tkdavaccount::clear {path} {
    variable state
    set state($path,url) ""
    set state($path,user) ""
...
proc ::tkutils::tkdavaccount::focusUrl {path} { focus $path.url; return $path.url }
...
proc ::tkutils::tkdavaccount::testConnection {path} {
    variable state
...
proc ::tkutils::tkdavaccount::_result {path ok message} {
    variable state
    if {$ok} {
...
package provide tkutils::tkdavaccount 0.1
