# tkutils::tkdavbrowser -- a read-only navigation pane for a CalDAV/CardDAV
...
#   tkdavbrowser::setClient .b $davClient
...
namespace eval ::tkutils::tkdavbrowser {
...
proc ::tkutils::tkdavbrowser::widget {path args} {
    variable state
    ...
    bind $path <Destroy> [list ::tkutils::tkdavbrowser::_cleanup $path %W]
    ...
    bind $tv <<TreeviewSelect>> [list ::tkutils::tkdavbrowser::_onSelect $path]

    if {$state($path,editable)} {
        set bb $path.bb
        ttk::frame $bb
        ttk::label $bb.nl -text "Name:"
        ttk::entry $bb.ne -width 16 -textvariable ::tkutils::tkdavbrowser::state($path,ename)
        ttk::button $bb.nc -text "New calendar" \
            -command [list ::tkutils::tkdavbrowser::_uiCreate $path calendar]
        ttk::button $bb.na -text "New address book" \
            -command [list ::tkutils::tkdavbrowser::_uiCreate $path addressbook]
        ttk::button $bb.del -text "Delete" \
            -command [list ::tkutils::tkdavbrowser::deleteCollection $path]
...
proc ::tkutils::tkdavbrowser::treeWidget {path} { return $path.tv }

proc ::tkutils::tkdavbrowser::setClient {path client} {
    variable state
...
proc ::tkutils::tkdavbrowser::refresh {path args} {
    variable state
...
proc ::tkutils::tkdavbrowser::setData {path cols} {
    variable state
...
proc ::tkutils::tkdavbrowser::_cleanup {path w} {
    variable state
    if {$w eq $path} { array unset state $path,* }
}

# --- provisioning (opt-in; network) ------------------------------------
proc ::tkutils::tkdavbrowser::_slug {name} {
...
proc ::tkutils::tkdavbrowser::_homeFor {path kind} {
    variable state
...
proc ::tkutils::tkdavbrowser::_requireClient {path} {
    variable state
...
proc ::tkutils::tkdavbrowser::createCalendar {path displayname} {
    return [_create $path calendar $displayname]
}
proc ::tkutils::tkdavbrowser::createAddressbook {path displayname} {
    return [_create $path addressbook $displayname]
}
proc ::tkutils::tkdavbrowser::_create {path kind displayname} {
    variable state
...
proc ::tkutils::tkdavbrowser::deleteCollection {path {href ""}} {
    variable state
    set client [_requireClient $path]
    if {$href eq ""} {
...
proc ::tkutils::tkdavbrowser::_uiCreate {path kind} {
    variable state
...
package provide tkutils::tkdavbrowser 0.1
