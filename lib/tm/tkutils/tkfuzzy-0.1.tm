# tkutils::tkfuzzy -- incremental fuzzy filter
#
...
proc ::tkutils::tkfuzzy::widget {path args} {
    ...
    bind $path <Destroy> [list ::tkutils::tkfuzzy::_cleanup $path %W]
    ...
    bind $path.e <KeyRelease> [list ::tkutils::tkfuzzy::_onType $path]
    return $path
}

proc ::tkutils::tkfuzzy::_onType {path} {
    filter $path [$path.e get]
}

# Set the list of items. Re-applies the current filter.
proc ::tkutils::tkfuzzy::setItems {path items} {
    variable state
...
proc ::tkutils::tkfuzzy::filter {path pattern} {
    variable state
    ...
    if {$pattern eq ""} {
        ...
        set ranked {}
        foreach pair [lsort -command ::tkutils::tkfuzzy::_cmp $scored] {
