# tkutils::tkical -- iCalendar event viewer/editor
#
...
proc ::tkutils::tkical::_cleanup {path w} {
    variable state
    if {$w eq $path} { array unset state $path,* }
}

proc ::tkutils::tkical::widget {path args} {
    variable state
    ...
    bind $path <Destroy> [list ::tkutils::tkical::_cleanup $path %W]
    ...
    if {$state($path,editable)} {
        ...
        ttk::entry $eb.se -width 16 -textvariable ::tkutils::tkical::state($path,esum)
        ttk::label $eb.al -text "Start:"
        ttk::entry $eb.ae -width 12 -textvariable ::tkutils::tkical::state($path,estart)
        ttk::label $eb.el -text "End:"
        ttk::entry $eb.ee -width 12 -textvariable ::tkutils::tkical::state($path,eend)
        ttk::label $eb.ll -text "Loc:"
        ttk::entry $eb.le -width 12 -textvariable ::tkutils::tkical::state($path,eloc)
        ttk::button $eb.set -text "Set"    -command [list ::tkutils::tkical::_uiSet $path]
        ttk::button $eb.add -text "Add"    -command [list ::tkutils::tkical::_uiAdd $path]
        ttk::button $eb.del -text "Delete" -command [list ::tkutils::tkical::_uiDel $path]
        ...
        bind $tv <<TreeviewSelect>> [list ::tkutils::tkical::_onSelect $path]
    }
    return $path
}

proc ::tkutils::tkical::treeWidget {path} { return $path.tv }

# Split parsed components into the calendar shell (props + non-event
# sub-components) and the editable list of events.
proc ::tkutils::tkical::_ingest {path comps} {
    variable state
    set cal ""
    foreach c $comps {
...
proc ::tkutils::tkical::_calComp {path} {
    variable state
...
proc ::tkutils::tkical::_populate {path} {
    variable state
    set tv $path.tv
...
proc ::tkutils::tkical::loadText {path ics} {
    variable state
    _ingest $path [::tclutils::tuical::parse $ics]
    return [llength $state($path,events)]
}

proc ::tkutils::tkical::loadFile {path file} {
    set ch [open $file r]
    set txt [read $ch]
    close $ch
...
proc ::tkutils::tkical::setComponents {path comps} {
    variable state
    _ingest $path $comps
    return [llength $state($path,events)]
}

proc ::tkutils::tkical::events {path} {
    variable state
    return $state($path,events)
}
proc ::tkutils::tkical::count {path} {
    variable state
    return [llength $state($path,events)]
}
...
proc ::tkutils::tkical::_fieldProp {field} {
    switch -- $field {
...
proc ::tkutils::tkical::addEvent {path {summary ""}} {
    variable state
...
proc ::tkutils::tkical::removeEvent {path index} {
    variable state
...
proc ::tkutils::tkical::setField {path index field value} {
    variable state
...
proc ::tkutils::tkical::setEventProperty {path index name value {params {}}} {
    variable state
    set ev [lindex $state($path,events) $index]
...
proc ::tkutils::tkical::addEventProperty {path index name value {params {}}} {
    variable state
    set ev [lindex $state($path,events) $index]
...
proc ::tkutils::tkical::removeEventProperty {path index name} {
    variable state
    set ev [lindex $state($path,events) $index]
...
proc ::tkutils::tkical::toText {path} {
    return [::tclutils::tuical::toIcs [_calComp $path]]
}
proc ::tkutils::tkical::save {path file} {
    set ch [open $file w]
...
proc ::tkutils::tkical::_onSelect {path} {
    variable state
    set tv $path.tv
...
proc ::tkutils::tkical::_uiSet {path} {
    variable state
...
proc ::tkutils::tkical::_uiAdd {path} {
    variable state
...
proc ::tkutils::tkical::_uiDel {path} {
    variable state
...
package provide tkutils::tkical 0.1
