# tkutils::tkimage -- image helpers and a scrollable/zoomable viewer widget for
...
#
#   tkimage::fit 1920 1080 800 600          ;# -> {800 450 0.41666} (contain math)
#   set thumb [tkimage::thumbnail $photo 96]
#   set v [tkimage::view .v]; pack $v -fill both -expand 1
#   tkimage::openFile .v photo.png          ;# load + fit to the widget
#   tkimage::zoomIn .v ; tkimage::zoomOut .v ; tkimage::zoom1 .v ; tkimage::fitView .v

package require Tk 8.6-

namespace eval ::tkutils {}
namespace eval ::tkutils::tkimage {
...
proc ::tkutils::tkimage::fit {imgW imgH boxW boxH {mode contain}} {
    if {$imgW <= 0 || $imgH <= 0} {
...
proc ::tkutils::tkimage::load {file {dst ""}} {
...
proc ::tkutils::tkimage::fromData {bytes {dst ""}} {
...
proc ::tkutils::tkimage::scale {src w h {dst ""}} {
...
proc ::tkutils::tkimage::view {path args} {
    ...
    bind $c    <Configure> [list ::tkutils::tkimage::_onResize $path]
    bind $path <Destroy>   [list ::tkutils::tkimage::_cleanup $path %W]
    return $path
}

proc ::tkutils::tkimage::openFile {path file} {
    variable state
...
proc ::tkutils::tkimage::getImage {path} {
    variable state
    return $state($path,orig)
}

proc ::tkutils::tkimage::zoomIn  {path} { _setZoom $path 1.25 }
proc ::tkutils::tkimage::zoomOut {path} { _setZoom $path 0.8 }
proc ::tkutils::tkimage::zoom1   {path} {
    variable state
    set state($path,zoom) 1.0
    set state($path,fitmode) actual
    _redraw $path
}
proc ::tkutils::tkimage::fitView {path} {
    variable state
    set state($path,fitmode) contain
    _redraw $path
}
proc ::tkutils::tkimage::_setZoom {path factor} {
    variable state
    set state($path,zoom) [expr {$state($path,zoom) * $factor}]
    set state($path,fitmode) actual
    _redraw $path
}
proc ::tkutils::tkimage::_onResize {path} {
    variable state
    if {[info exists state($path,fitmode)] && $state($path,fitmode) eq "contain"} {
        _redraw $path
    }
}
proc ::tkutils::tkimage::_redraw {path} {
    variable state
...
proc ::tkutils::tkimage::_cleanup {path w} {
    variable state
    if {$w ne $path} { return }
...
package provide tkutils::tkimage 0.1
