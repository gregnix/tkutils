...
proc ::tkutils::tkini::_cleanup {path w} {
    variable state
    if {$w eq $path} { array unset state $path,* }
}

proc ::tkutils::tkini::widget {path args} {
    variable state
    ...
    set state($path,eval) ""
    set state($path,esec) ""
    bind $path <Destroy> [list ::tkutils::tkini::_cleanup $path %W]
    ...
    if {$state($path,editable)} {
        ...
        ttk::entry $eb.se -width 12 -textvariable ::tkutils::tkini::state($path,esec)
        ttk::button $eb.sa -text "Add Sec" -command [list ::tkutils::tkini::_uiAddSection $path]
        ttk::button $eb.sd -text "Del Sec" -command [list ::tkutils::tkini::_uiDelSection $path]
        ttk::separator $eb.sep -orient vertical
        ttk::label $eb.kl -text "Key:"
        ttk::entry $eb.ke -width 14 -textvariable ::tkutils::tkini::state($path,ekey)
        ttk::label $eb.vl -text "Value:"
        ttk::entry $eb.ve -width 20 -textvariable ::tkutils::tkini::state($path,eval)
        ttk::button $eb.set -text "Set" -command [list ::tkutils::tkini::_uiSet $path]
        ttk::button $eb.del -text "Delete" -command [list ::tkutils::tkini::_uiDel $path]
        ...
        bind $kv <<TreeviewSelect>> [list ::tkutils::tkini::_fillEdit $path]
    }

    pack $pw -side top -fill both -expand 1

    bind $sec <<TreeviewSelect>> [list ::tkutils::tkini::_onSelect $path]
    return $path
}

proc ::tkutils::tkini::sectionWidget {path} { return $path.pw.lf.tv }
proc ::tkutils::tkini::treeWidget {path}    { return $path.pw.rf.tv }

proc ::tkutils::tkini::_label {section} {
    return [expr {$section eq "" ? "(global)" : $section}]
}

proc ::tkutils::tkini::_populateSections {path} {
    variable state
...
proc ::tkutils::tkini::_showSection {path section} {
    variable state
...
proc ::tkutils::tkini::_onSelect {path} {
    variable state
    set sec [sectionWidget $path]
...
proc ::tkutils::tkini::_itemFor {path section} {
    variable state
...
proc ::tkutils::tkini::loadText {path ini} {
    variable state
...
proc ::tkutils::tkini::loadFile {path file} {
    set ch [open $file r]
    set txt [read $ch]
    close $ch
...
proc ::tkutils::tkini::setData {path d} {
    variable state
...
proc ::tkutils::tkini::selectSection {path section} {
    set sec [sectionWidget $path]
...
proc ::tkutils::tkini::data {path} {
    variable state
    return $state($path,data)
}
proc ::tkutils::tkini::sections {path} {
    variable state
    return [dict keys $state($path,data)]
}
proc ::tkutils::tkini::currentSection {path} {
    variable state
    return $state($path,cur)
}
proc ::tkutils::tkini::count {path} {
    variable state
    return [dict size $state($path,data)]
}
...
proc ::tkutils::tkini::_reload {path} {
    variable state
    _populateSections $path
...
proc ::tkutils::tkini::setKey {path section key value} {
    variable state
...
proc ::tkutils::tkini::removeSection {path section} {
    variable state
...
proc ::tkutils::tkini::_fillEdit {path} {
    variable state
    set kv [treeWidget $path]
...
proc ::tkutils::tkini::_uiDelSection {path} {
    variable state
    removeSection $path $state($path,cur)
}

package provide tkutils::tkini 0.1
