# tkutils::tkjson -- JSON tree viewer
#
...
if {![llength [info commands ::tclutils::tujson::parseTyped]]} {
    return -code error -errorcode {TKUTILS TKJSON DEP} \
        "tkutils::tkjson requires tclutils::tujson with parseTyped (tclutils 0.28.0+)"
}

namespace eval ::tkutils {}
namespace eval ::tkutils::tkjson {
    namespace export widget loadFile setJson getTree
    variable state
}

proc ::tkutils::tkjson::_cleanup {path w} {
    variable state
    if {$w eq $path} { array unset state $path,* }
}

# Build the tree widget under $path. Option: -height N (visible rows).
proc ::tkutils::tkjson::widget {path args} {
    variable state
    ...
    ttk::frame $path
    set state($path,tree) {}
    bind $path <Destroy> [list ::tkutils::tkjson::_cleanup $path %W]
...
proc ::tkutils::tkjson::setJson {path jsonText} {
    variable state
...
proc ::tkutils::tkjson::loadFile {path filename} {
    variable state
    set state($path,tree) \
...
proc ::tkutils::tkjson::getTree {path} {
    variable state
    return $state($path,tree)
}

proc ::tkutils::tkjson::_populate {path} {
    variable state
    set tv $path.tv
...
proc ::tkutils::tkjson::_insert {tv parent label node} {
...
package provide tkutils::tkjson 0.1
