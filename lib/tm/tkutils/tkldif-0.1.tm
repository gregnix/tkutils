# tkutils::tkldif -- LDIF entry viewer/editor
#
...
proc ::tkutils::tkldif::_cleanup {path w} {
    variable state
    if {$w eq $path} { array unset state $path,* }
}

proc ::tkutils::tkldif::widget {path args} {
    variable state
    ...
    set state($path,eval) ""
    bind $path <Destroy> [list ::tkutils::tkldif::_cleanup $path %W]
    ...
    if {$state($path,editable)} {
        ...
        ttk::entry $eb.ae -width 14 -textvariable ::tkutils::tkldif::state($path,eattr)
        ttk::label $eb.vl -text "Value:"
        ttk::entry $eb.ve -width 24 -textvariable ::tkutils::tkldif::state($path,eval)
        ttk::button $eb.set -text "Set" -command [list ::tkutils::tkldif::_uiSet $path]
        ttk::button $eb.add -text "Add" -command [list ::tkutils::tkldif::_uiAdd $path]
        ttk::button $eb.del -text "Del" -command [list ::tkutils::tkldif::_uiDel $path]
        ttk::separator $eb.sep -orient vertical
        ttk::button $eb.ae2 -text "Add Entry" -command [list ::tkutils::tkldif::_uiAddEntry $path]
        ttk::button $eb.de2 -text "Del Entry" -command [list ::tkutils::tkldif::_uiDelEntry $path]
        ...
        bind $tv <<TreeviewSelect>> [list ::tkutils::tkldif::_onSelect $path]
    }
    return $path
}

proc ::tkutils::tkldif::treeWidget {path} { return $path.tv }

proc ::tkutils::tkldif::_populate {path} {
    variable state
    set tv $path.tv
...
proc ::tkutils::tkldif::loadText {path ldif} {
    variable state
...
proc ::tkutils::tkldif::loadFile {path file} {
    set ch [open $file r]
    set txt [read $ch]
    close $ch
...
proc ::tkutils::tkldif::setEntries {path entries} {
    variable state
...
proc ::tkutils::tkldif::entries {path} {
    variable state
    return $state($path,entries)
}
proc ::tkutils::tkldif::count {path} {
    variable state
    return [llength $state($path,entries)]
}
...
proc ::tkutils::tkldif::setAttr {path entryIndex pairIndex attr value} {
    variable state
    set e [lindex $state($path,entries) $entryIndex]
...
proc ::tkutils::tkldif::removeAttr {path entryIndex pairIndex} {
    variable state
    set e [lindex $state($path,entries) $entryIndex]
...
proc ::tkutils::tkldif::toText {path} {
    variable state
    return [::tclutils::tuldif::toLdif $state($path,entries)]
}
proc ::tkutils::tkldif::save {path file} {
    set ch [open $file w]
...
proc ::tkutils::tkldif::_onSelect {path} {
    variable state
    set tv $path.tv
...
proc ::tkutils::tkldif::_uiSet {path} {
    variable state
...
proc ::tkutils::tkldif::_uiAddEntry {path} {
    variable state
    set dn [expr {$state($path,eval) ne "" ? $state($path,eval) : "cn=New"}]
    addEntry $path $dn
}
proc ::tkutils::tkldif::_uiDelEntry {path} {
    variable state
...
package provide tkutils::tkldif 0.1
