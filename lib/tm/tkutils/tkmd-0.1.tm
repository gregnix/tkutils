# tkutils::tkmd -- Markdown outline viewer
#
...
proc ::tkutils::tkmd::widget {path args} {
    ...
    bind $path <Destroy> [list ::tkutils::tkmd::_cleanup $path %W]
...
proc ::tkutils::tkmd::setMarkdown {path markdown} {
    variable state
...
proc ::tkutils::tkmd::loadFile {path filename} {
    variable state
    set state($path,headings) \
...
proc ::tkutils::tkmd::getHeadings {path} {
    variable state
    return $state($path,headings)
}

proc ::tkutils::tkmd::_populate {path} {
    variable state
    set tv $path.tv
...
package provide tkutils::tkmd 0.1
