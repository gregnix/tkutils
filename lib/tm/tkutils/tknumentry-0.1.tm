# tkutils::tknumentry -- numeric entry with input validation, fixed decimals
...
namespace eval ::tkutils::tknumentry {
    namespace export widget getValue setValue clear
    variable state
}

proc ::tkutils::tknumentry::_cleanup {path w} {
    variable state
...
proc ::tkutils::tknumentry::widget {path args} {
    variable state
    ...
    bind $path <Destroy> [list ::tkutils::tknumentry::_cleanup $path %W]

    set evar [list]
    if {$opts(-textvariable) ne ""} { set evar [list -textvariable $opts(-textvariable)] }
    ttk::entry $path.e -width $opts(-width) -justify right {*}$evar \
        -validate key \
        -validatecommand [list ::tkutils::tknumentry::_validate $path %P]
    grid $path.e -sticky ew
    grid columnconfigure $path 0 -weight 1
    bind $path.e <Return>   [list ::tkutils::tknumentry::_commit $path]
    bind $path.e <FocusOut> [list ::tkutils::tknumentry::_commit $path]
    return $path
}
...
proc ::tkutils::tknumentry::getValue {path} {
    set s [string trim [$path.e get]]
    if {$s eq "" || $s eq "-" || $s eq "."} { return "" }
    return $s
}

proc ::tkutils::tknumentry::setValue {path value} {
    variable state
...
proc ::tkutils::tknumentry::clear {path} { _replace $path ""; return "" }

# --- internals -----------------------------------------------------------

proc ::tkutils::tknumentry::_replace {path text} {
...
# fractional digits.
proc ::tkutils::tknumentry::_validate {path proposed} {
    variable state
...
proc ::tkutils::tknumentry::_clamp {path value} {
    variable state
...
proc ::tkutils::tknumentry::_commit {path} {
    variable state
    set s [getValue $path]
    if {$s ne ""} {
        set v [_clamp $path $s]
...
package provide tkutils::tknumentry 0.1
