# tkutils::tksqlite -- SQLite table browser
#
...
# package -- load it directly with `package require tkutils::tksqlite` once
...
namespace eval ::tkutils::tksqlite {
    namespace export widget openFile tables showTable getRows closeDb
    variable state
}

proc ::tkutils::tksqlite::_cleanup {path w} {
    variable state
    if {$w ne $path} return
    closeDb $path
...
proc ::tkutils::tksqlite::_handle {path} {
    return ::tkutils::tksqlite::db_[string map {. _ : _} $path]
}

# Build the browser under $path. Option: -height N (visible rows).
proc ::tkutils::tksqlite::widget {path args} {
    variable state
    ...
    set state($path,rows) {}
    bind $path <Destroy> [list ::tkutils::tksqlite::_cleanup $path %W]
    ...
    bind $path.pw.left.lb <<ListboxSelect>> \
        [list ::tkutils::tksqlite::_onSelect $path]
    return $path
}

proc ::tkutils::tksqlite::_onSelect {path} {
...
proc ::tkutils::tksqlite::openFile {path dbfile} {
    variable state
    closeDb $path
    set db [_handle $path]
    sqlite3 $db $dbfile
...
proc ::tkutils::tksqlite::tables {path} {
    variable state
    return $state($path,tables)
}

# Show the rows of $table. Returns the row count.
proc ::tkutils::tksqlite::showTable {path table} {
    variable state
...
proc ::tkutils::tksqlite::getRows {path} {
    variable state
    return $state($path,rows)
}

proc ::tkutils::tksqlite::closeDb {path} {
    variable state
...
package provide tkutils::tksqlite 0.1
