# tkutils::tkstrings -- printable strings viewer
#
...
proc ::tkutils::tkstrings::widget {path args} {
    ...
    bind $path <Destroy> [list ::tkutils::tkstrings::_cleanup $path %W]
...
proc ::tkutils::tkstrings::setData {path data args} {
    variable state
...
proc ::tkutils::tkstrings::loadFile {path filename args} {
    variable state
...
proc ::tkutils::tkstrings::getStrings {path} {
    variable state
    return $state($path,strings)
}

proc ::tkutils::tkstrings::_populate {path} {
    variable state
    $path.lb delete 0 end
    foreach s $state($path,strings) { $path.lb insert end $s }
}

package provide tkutils::tkstrings 0.1
