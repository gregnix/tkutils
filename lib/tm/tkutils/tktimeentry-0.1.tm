# tkutils::tktimeentry -- time entry with hour/minute (optional second) spinboxes.
...
namespace eval ::tkutils::tktimeentry {
    namespace export widget getTime setTime
    variable state
}

proc ::tkutils::tktimeentry::_cleanup {path w} {
    variable state
...
proc ::tkutils::tktimeentry::widget {path args} {
    variable state
    ...
    set state($path,m) 0
    set state($path,s) 0
    bind $path <Destroy> [list ::tkutils::tktimeentry::_cleanup $path %W]

    set ns ::tkutils::tktimeentry::state
    set chg [list ::tkutils::tktimeentry::_onChange $path]
...
proc ::tkutils::tktimeentry::_int {v} {
    set v [string trim $v]
    if {[regexp {^\d+$} $v]} { return [scan $v %d] }
    return 0
}

proc ::tkutils::tktimeentry::getTime {path} {
    variable state
...
proc ::tkutils::tktimeentry::setTime {path str} {
    variable state
    set parts [split $str :]
...
proc ::tkutils::tktimeentry::_onChange {path} {
    variable state
...
package provide tkutils::tktimeentry 0.1
