# tkutils::tktodo -- a task-list widget for iCalendar VTODO components, built on
...
#
#   set w [tktodo::widget .t]
#   pack $w -fill both -expand 1
#   tktodo::loadText .t $ics            ;# parse + extract all VTODOs
#   tktodo::toggle .t                   ;# flip the selected task's done state
#   set ics [tuical::toIcs [tktodo::todos .t]]   ;# write changes back
...
proc ::tkutils::tktodo::widget {path args} {
    variable state
    ...
    bind $path <Destroy> [list ::tkutils::tktodo::_cleanup $path %W]
    ...
    bind $tv <<TreeviewSelect>> [list ::tkutils::tktodo::_onSelect $path]
    if {$state($path,editable)} {
        bind $tv <Double-1>  [list ::tkutils::tktodo::_onActivate $path]
        bind $tv <space>     [list ::tkutils::tktodo::_onActivate $path]
        set bb $path.bb
        ttk::frame $bb
        ttk::button $bb.toggle -text "Toggle done" \
            -command [list ::tkutils::tktodo::toggle $path]
...
proc ::tkutils::tktodo::treeWidget {path} { return $path.tv }

proc ::tkutils::tktodo::setTodos {path comps} {
    variable state
...
proc ::tkutils::tktodo::_onSelect {path} {
    variable state
...
proc ::tkutils::tktodo::_onActivate {path} {
    toggle $path
}

# Flip the done state of the selected task (or the task at $index).
proc ::tkutils::tktodo::toggle {path {index ""}} {
    variable state
...
proc ::tkutils::tktodo::_setDone {path index done} {
    variable state
    set comp [lindex $state($path,todos) $index]
    if {$done} {
...
proc ::tkutils::tktodo::_cleanup {path w} {
    variable state
    if {$w eq $path} { array unset state $path,* }
}

package provide tkutils::tktodo 0.1
