# tkutils::tktree -- a thin ttk::treeview wrapper for hierarchical data.
...
proc ::tkutils::tktree::widget {path args} {
    variable state
    ...
    bind $path <Destroy> [list ::tkutils::tktree::_cleanup $path %W]
    ...
    bind $path.t <<TreeviewSelect>> [list ::tkutils::tktree::_select $path]
    return $path
}
...
proc ::tkutils::tktree::insert {path parent text args} {
    variable state
...
proc ::tkutils::tktree::loadTree {path nodes {parent ""}} {
    foreach node $nodes {
...
proc ::tkutils::tktree::clear {path} {
    $path.t delete [$path.t children ""]
    return
}

proc ::tkutils::tktree::selection {path} {
    return [$path.t selection]
}

proc ::tkutils::tktree::itemText {path id} {
    return [$path.t item $id -text]
}

proc ::tkutils::tktree::itemValues {path id} {
    return [$path.t item $id -values]
}

proc ::tkutils::tktree::children {path {id ""}} {
    return [$path.t children $id]
}

proc ::tkutils::tktree::treeview {path} {
    return $path.t
}

# --- internals -----------------------------------------------------------

proc ::tkutils::tktree::_select {path} {
    variable state
...
package provide tkutils::tktree 0.1
