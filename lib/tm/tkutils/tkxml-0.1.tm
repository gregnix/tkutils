...
# it directly with `package require tkutils::tkxml` once tdom is available.
...
namespace eval ::tkutils::tkxml {
    namespace export widget setXml loadFile getRoot
    variable state
}

proc ::tkutils::tkxml::_cleanup {path w} {
    variable state
    if {$w ne $path} return
...
proc ::tkutils::tkxml::widget {path args} {
    variable state
    ...
    ttk::frame $path
    set state($path,doc) ""
    bind $path <Destroy> [list ::tkutils::tkxml::_cleanup $path %W]
...
proc ::tkutils::tkxml::setXml {path xml} {
    variable state
...
proc ::tkutils::tkxml::loadFile {path filename} {
    variable state
...
proc ::tkutils::tkxml::_insertNode {tv parent node} {
    set attrs {}
...
package provide tkutils::tkxml 0.1
